-- ===========================================================================
-- BOLETINES: además de imágenes, video (mp4) y audio (mp3)
--
-- Cada fila del carrusel dice qué es. El tipo no se lo cree a la pantalla:
-- sale de la extensión del fichero, que es lo que el controlador valida al
-- subirlo. Lo que ya estaba queda como IMAGEN.
--
--   psql -h 192.168.2.173 -U postgres -d crm3 -f 2026-09-21_core_boletines_media.sql
-- ===========================================================================
...
UPDATE core.boletines_imagenes
   SET tipo = core.fn_boletines_tipo_archivo(archivo)
 WHERE tipo IS DISTINCT FROM core.fn_boletines_tipo_archivo(archivo);

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'boletines_imagenes_tipo_check') THEN
        ALTER TABLE core.boletines_imagenes
            ADD CONSTRAINT boletines_imagenes_tipo_check CHECK (tipo IN ('IMAGEN', 'VIDEO', 'AUDIO'));
    END IF;
END $$;

COMMENT ON COLUMN core.boletines_imagenes.tipo IS 'IMAGEN, VIDEO o AUDIO: lo decide la extensión del fichero';
