-- ============================================================================
-- ACCESO «papelera»: la columna seguridad.accesos.papelera (boolean, false)
-- da acceso al botón de Papelera de reciclaje de cada componente, igual que
-- crear / editar / eliminar / auditar. Aquí se añade a fn_perfiles_crear y
-- fn_perfiles_modificar (alta de accesos y json de auditoría); el back
-- (ProfileController) la lee y la guarda, y el front la pinta en el editor
-- de perfiles y la usa en los componentes (accesoModel.papelera).
-- Generado a partir de las funciones actuales. Idempotente: CREATE OR REPLACE.
-- ============================================================================
ALTER TABLE seguridad.accesos ADD COLUMN IF NOT EXISTS papelera boolean NOT NULL DEFAULT false;
COMMENT ON COLUMN seguridad.accesos.papelera IS 'Puede abrir la papelera de reciclaje del componente (restaurar / borrar definitivamente)';

CREATE OR REPLACE FUNCTION seguridad.fn_perfiles_crear(p_nombre character varying, p_inactividad integer, p_activo boolean, p_accesos jsonb, p_usuario_id bigint DEFAULT NULL::bigint, p_usuario_login character varying DEFAULT NULL::character varying, p_usuario_nombre character varying DEFAULT NULL::character varying, p_ip_address inet DEFAULT NULL::inet, p_user_agent text DEFAULT NULL::text, p_request_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
    ...
    SELECT jsonb_agg(
        jsonb_build_object(
            ...
            'permisos', jsonb_build_object(
                ...
                'papelera', COALESCE((elem->>'papelera')::BOOLEAN, false)
            )
        )
    ...
    FOR v_acceso IN SELECT * FROM jsonb_array_elements(p_accesos) LOOP
        INSERT INTO seguridad.accesos (
            ...
            COALESCE((v_acceso->>'papelera')::BOOLEAN, false)
        );
    END LOOP;
...
CREATE OR REPLACE FUNCTION seguridad.fn_perfiles_modificar(p_id bigint, p_nombre character varying, p_inactividad integer, p_activo boolean, p_accesos jsonb, p_usuario_id bigint DEFAULT NULL::bigint, p_usuario_login character varying DEFAULT NULL::character varying, p_usuario_nombre character varying DEFAULT NULL::character varying, p_ip_address inet DEFAULT NULL::inet, p_user_agent text DEFAULT NULL::text, p_request_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
    ...
    IF p_accesos IS NOT NULL THEN
        ...
        FOR v_item IN SELECT * FROM jsonb_array_elements(p_accesos) LOOP
            INSERT INTO seguridad.accesos (
                ...
                COALESCE((v_item.value->>'papelera')::BOOLEAN, false)
            );
        END LOOP;
    END IF;
