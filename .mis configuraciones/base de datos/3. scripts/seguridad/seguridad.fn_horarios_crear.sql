-- FUNCTION: seguridad.fn_horarios_crear(character varying, boolean, jsonb, bigint, character varying, character varying, inet, text, uuid)

-- DROP FUNCTION IF EXISTS seguridad.fn_horarios_crear(character varying, boolean, jsonb, bigint, character varying, character varying, inet, text, uuid);

CREATE OR REPLACE FUNCTION seguridad.fn_horarios_crear(
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM seguridad.chorarios WHERE nombre = TRIM(p_nombre)) THEN
        RETURN jsonb_build_object('success', false, 'message', 'Ya existe un horario con ese nombre', 'error_code', 'NOMBRE_DUPLICADO');
    END IF;
    
    -- Insertar cabecera
    INSERT INTO seguridad.chorarios (nombre, activo)
    VALUES (TRIM(p_nombre), COALESCE(p_activo, true))
    RETURNING id INTO v_horario_id;
    
    -- Insertar dhorario si existe
    ...
    -- Obtener el horario completo con dhorario
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN jsonb_build_object(
            'success', false,
            'message', 'Error al crear horario: ' || SQLERRM,
            'error_code', SQLSTATE
        );
END;
$BODY$;

ALTER FUNCTION seguridad.fn_horarios_crear(character varying, boolean, jsonb, bigint, character varying, character varying, inet, text, uuid)
    OWNER TO postgres;
