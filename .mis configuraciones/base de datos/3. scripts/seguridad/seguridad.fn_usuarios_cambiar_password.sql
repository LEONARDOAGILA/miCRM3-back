-- FUNCTION: seguridad.fn_usuarios_cambiar_password(bigint, character varying, boolean, bigint, character varying, character varying, inet, text, uuid)

-- DROP FUNCTION IF EXISTS seguridad.fn_usuarios_cambiar_password(bigint, character varying, boolean, bigint, character varying, character varying, inet, text, uuid);
...
BEGIN
    ...
    IF p_password IS NULL OR TRIM(p_password) = '' THEN
        RETURN jsonb_build_object('success', false, 'message', 'La contraseña es obligatoria', 'error_code', 'PASSWORD_REQUERIDO');
    END IF;
    ...
    -- 5. Obtener datos del perfil
    SELECT id, nombre INTO v_perfil_data
    FROM seguridad.perfiles 
    WHERE id = v_usuario_actual.perfil_id;
    
    -- 6. Obtener datos del horario
    SELECT id, nombre INTO v_chorario_data
    FROM seguridad.chorarios 
    WHERE id = v_usuario_actual.chorario_id;
    ...
    -- 9. Construir JSON de datos NUEVOS (con la nueva contraseña)
    -- NOTA: La contraseña se guarda en texto plano en auditoría (ya viene encriptada desde Laravel)
    v_datos_nuevos := jsonb_build_object(
        ...
        'isactive', v_usuario_actual.isactive,
        'islogin', v_usuario_actual.islogin,
        'isreset', COALESCE(p_isreset, v_usuario_actual.isreset),
        'perfil', jsonb_build_object(
            'id', v_perfil_data.id,
            'nombre', v_perfil_data.nombre
        ),
    ...
    -- 11. Actualizar contraseña y isreset
    UPDATE seguridad.users
    SET 
        password = p_password,
        isreset = COALESCE(p_isreset, isreset),
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RETURN jsonb_build_object(
            'success', false,
            'message', 'Error al cambiar contraseña: ' || SQLERRM,
            'error_code', SQLSTATE
        );
END;
$BODY$;
