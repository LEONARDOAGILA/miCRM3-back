-- FUNCTION: seguridad.fn_usuarios_listar_paginado_activos(integer, integer, text)

-- DROP FUNCTION IF EXISTS seguridad.fn_usuarios_listar_paginado_activos(integer, integer, text);

CREATE OR REPLACE FUNCTION seguridad.fn_usuarios_listar_paginado_activos(
...
BEGIN
    ...
    IF p_search IS NOT NULL AND p_search != '' THEN
        ...
        WHERE u.isactive = true  -- 👈 SOLO ACTIVOS
          AND (
            u.login_user ILIKE '%' || p_search || '%' 
    ...
    -- 2. Consulta paginada (solo activos)
    IF p_search IS NOT NULL AND p_search != '' THEN
        SELECT COALESCE(
            (SELECT jsonb_agg(
                jsonb_build_object(
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN jsonb_build_object(
            'success', false,
            'message', 'Error al listar usuarios activos: ' || SQLERRM,
            'error_code', SQLSTATE
        );
END;
$BODY$;

ALTER FUNCTION seguridad.fn_usuarios_listar_paginado_activos(integer, integer, text)
    OWNER TO postgres;