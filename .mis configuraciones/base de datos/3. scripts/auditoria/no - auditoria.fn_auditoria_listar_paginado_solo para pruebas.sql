-- FUNCTION: auditoria.fn_auditoria_listar_paginado(text, bigint, text, text, text, integer, integer)

-- DROP FUNCTION IF EXISTS auditoria.fn_auditoria_listar_paginado(text, bigint, text, text, text, integer, integer);

CREATE OR REPLACE FUNCTION auditoria.fn_auditoria_listar_paginado(
	p_tabla_nombre text DEFAULT NULL::text,
	p_registro_id bigint DEFAULT NULL::bigint,
...
BEGIN
    ...
    WHERE (p_tabla_nombre IS NULL OR tabla_nombre = p_tabla_nombre)
      AND (p_registro_id IS NULL OR registro_id = p_registro_id)
...
ALTER FUNCTION auditoria.fn_auditoria_listar_paginado(text, bigint, text, text, text, integer, integer)
    OWNER TO postgres;
